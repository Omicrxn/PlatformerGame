...
void Render::CameraUpdate(iPoint position) 
{
	int xAxis = (-position.x * scale) + (win->GetWindowWidth() / 2);
	int yAxis = (-position.y * scale) + ((win->GetWindowHeight() / 2));
...
// Draw to screen
bool Render::DrawTexture(SDL_Texture* texture, int x, int y, const SDL_Rect* section, float speed, bool isLeft, double angle, int pivotX, int pivotY) const
{
	bool ret = true;
